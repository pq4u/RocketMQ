...
mTLS uwierzytelnia klienta. Opcjonalna autoryzacja mapuje SHA-256 certyfikatu na stabilny <code>ClientId</code> i niezależne role dla Producer, Consumer oraz Admin. Walidacja łańcucha nie jest powtarzana przy RPC; handler aplikacyjny oblicza fingerprint i tworzy claims. Uprawnienia są obecnie globalne dla całej grupy operacji, bez ACL per exchange/kolejka. Broker nadal nie ma limitów per klient ani trwałego audytu bezpieczeństwa. Jawny tryb HTTP jest dozwolony wyłącznie na loopback, gdy mTLS jest wyłączone.
