...
mTLS powoduje, że broker nie tylko przedstawia swój certyfikat TLS, ale również żąda certyfikatu od klienta. W RocketMQ jest to funkcja opcjonalna. Każdy poprawny certyfikat klienta podpisany przez skonfigurowane CA uzyskuje dostęp do wszystkich RPC; role i ACL nie są jeszcze dostępne.
...
Benchmark przyjmuje ścieżkę PFX, ale nazwę zmiennej z hasłem przekazuje osobno, aby sekret nie trafiał do argumentów procesu:
...
Zmiana CA lub certyfikatu klienta wymaga restartu procesu i utworzenia nowych połączeń.
