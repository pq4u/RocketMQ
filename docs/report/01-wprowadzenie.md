...
- opcjonalną autoryzację operacji Publish, Consume i Admin, globalnie lub per exchange/kolejka;
...
Prototyp nie realizuje klastra, replikacji ani dokładnie jednokrotnego przetwarzania. Domyślny TLS nie identyfikuje klienta, natomiast opcjonalne mTLS dopuszcza tylko certyfikaty prowadzące do skonfigurowanego prywatnego CA. Dodatkowa allowlista mapuje fingerprint certyfikatu na klienta i globalne lub zasobowe role Publish, Consume oraz Admin. ACL używa dokładnych nazw; nie obsługuje wildcardów, tenantów ani zmiany bez restartu. Nie ukończono również własnego adaptera WAL.
