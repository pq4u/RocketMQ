...
Use [`tools/RocketMQ.Benchmark`](../../tools/RocketMQ.Benchmark/README.md) against a separately running broker and a fresh SQLite database for every comparable run. Its default scenario is the WAL decision baseline: 30-second warm-up, 15 minutes, direct routing to one queue, 32 closed-loop workers, and 1 KiB payloads. It records completed durable `Publish` operations only and writes a JSON report with throughput, latency percentiles, errors, and SQLite file sizes. Run this baseline three times before profiling or considering custom WAL.
